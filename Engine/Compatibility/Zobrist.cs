...
public static class Zobrist
{
    ...
    public static readonly ulong[,,] piecesArray = new ulong[6, 2, 64];
    public static readonly ulong[] castlingArray = new ulong[16];
    public static readonly ulong[] epArray = new ulong[8];
   ...
   0xF8D626AAAF278509,
    ...
    static Zobrist()
    {
        //Initialize piece randoms
        for (int pieceIndex = 0; pieceIndex < 6; pieceIndex++)
        {
            int pieceWhite = (pieceIndex + 1) | Piece.White;
            int pieceBlack = (pieceIndex + 1) | Piece.Black;

            int convertedPieceWhite = PieceToPolyGlot(pieceWhite);
            int convertedPieceBlack = PieceToPolyGlot(pieceBlack);

            for (int squareIndex = 0; squareIndex < 64; squareIndex++)
            {
                int whitePieceOffset = 64 * convertedPieceWhite + squareIndex;
                int blackPieceOffset = 64 * convertedPieceBlack + squareIndex;

                piecesArray[pieceIndex, 0, squareIndex] = PolyglotRandoms[whitePieceOffset];
                piecesArray[pieceIndex, 1, squareIndex] = PolyglotRandoms[blackPieceOffset];
            }
        }

        //Initialize castle randoms
        for (int castleValue = 0; castleValue < 16; castleValue++)
        {
            ulong random = 0;

            if ((castleValue & 0b0001) != 0) //White can shortcastle
            {
                random ^= PolyglotRandoms[768];
            }
            if ((castleValue & 0b0010) != 0) //black can shortcastle
            {
                random ^= PolyglotRandoms[768 + 2];
            }
            if ((castleValue & 0b0100) != 0) //white can longcastle
            {
                random ^= PolyglotRandoms[768 + 1];
            }
            if ((castleValue & 0b1000) != 0) //black can longcastle
            {
                random ^= PolyglotRandoms[768 + 3];
            }

            castlingArray[castleValue] = random;
        }

        //Initialize ep randoms
        for (int epFile = 0; epFile < 8; epFile++)
        {
            epArray[epFile] = PolyglotRandoms[772 + epFile];
        }

        //Intialize side to move
        sideToMove = PolyglotRandoms[780];
    }
    ...
    public static ulong Hash(Board board)
    {
        ...
        for (int typeIndex = 0; typeIndex < 5; typeIndex++)
        {
            ...
            {
                int square = whitePieceList[i];
                key ^= piecesArray[typeIndex + 1, 0, square];
            }

            for (int i = 0; i < blackPieceList.Count; i++)
            {
                int square = blackPieceList[i];
                key ^= piecesArray[typeIndex + 1, 1, square];
            }
        }
